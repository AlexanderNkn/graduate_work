...
async def test_pagination_first_page_size(send_data_to_elastic, person_list, make_get_request, person_list_expected):
    # testing scenario:
...
async def test_pagination_second_page_size(send_data_to_elastic, person_list, make_get_request, person_list_expected):
    # testing scenario:
    # we select page size that there are n-1 record on first page and 1 record on second page
...
async def test_pagination_page_size_negative(send_data_to_elastic, person_list, make_get_request):
    async with send_data_to_elastic(data=person_list):
        response = await make_get_request('/person?page[size]=-1')
        assert response.status == HTTPStatus.BAD_REQUEST


async def test_pagination_page_size_not_number(send_data_to_elastic, person_list, make_get_request):
    async with send_data_to_elastic(data=person_list):
        response = await make_get_request('/person?page[size]=a')
        assert response.status == HTTPStatus.BAD_REQUEST


async def test_person_text_search_by_name(send_data_to_elastic, person_list, make_get_request):
    async with send_data_to_elastic(data=person_list):
...
async def test_person_text_search_by_role(send_data_to_elastic, person_list, make_get_request):
    async with send_data_to_elastic(data=person_list):
