...
async def test_get_cached_genre(
    ...
    es_response = await es_client.get(index='genres', id='32345678-1234-1234-1234-123456789101',
...
async def test_pagination_first_page_size(send_data_to_elastic, genre_list, make_get_request, genre_list_expected):
    # testing scenario:
...
async def test_pagination_second_page_size(send_data_to_elastic, genre_list, make_get_request, genre_list_expected):
    # testing scenario:
    # we select page size that there are n-1 record on first page and 1 record on second page
...
async def test_pagination_page_size_negative(send_data_to_elastic, genre_list, make_get_request):
    async with send_data_to_elastic(data=genre_list):
        response = await make_get_request('/genre?page[size]=-1')
        assert response.status == HTTPStatus.BAD_REQUEST


async def test_pagination_page_size_not_number(send_data_to_elastic, genre_list, make_get_request):
    async with send_data_to_elastic(data=genre_list):
        response = await make_get_request('/genre?page[size]=a')
        assert response.status == HTTPStatus.BAD_REQUEST


async def test_genre_text_search_by_title(send_data_to_elastic, genre_list, make_get_request):
    async with send_data_to_elastic(data=genre_list):
...
async def test_genre_text_search_by_description(send_data_to_elastic, genre_list, make_get_request):
    async with send_data_to_elastic(data=genre_list):
